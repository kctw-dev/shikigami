...
## 禁用軟性字樣清單（#994，Sprint 182）

PO Round 1 輸出 AC 表格前，**必須**執行自檢步驟確保 AC 不含軟性字樣。軟性字樣缺乏精確度，易導致 QA 與開發無法精準理解需求。

### 禁用字樣表

| 禁用字樣 | 定義 | 反例（禁用） | 正例（改寫） | 改寫類型 |
|---------|------|-----------|----------|---------|
| 考慮 | 沒有明確指示的模糊表述 | 功能應考慮使用者體驗 | 當螢幕寬度 < 480px 時，折疊側欄 | 條件 |
| 明確 | 模糊的確認度指示 | AC 應該明確定義 | AC 必須包含所有邊界條件（state、HTTP code、異常） | 狀態 |
| 適當 | 沒有衡量標準的判斷詞 | 採用適當的設計模式 | 使用 Factory Pattern 實現 dependency injection | 路徑 |
| 合理 | 主觀判斷詞 | 合理的效能優化 | 延遲加載時間 <= 200ms（基準：P95 latency） | 數值 |
| 盡量 | 表示最大努力但無保證 | 盡量減少 API 呼叫 | 將 API 呼叫次數限制在 1 次（per user action） | 數值 |
| 儘可能 | 表示盡可能但無承諾 | 儘可能快速回應 | 回應時間 < 100ms（99 percentile） | 數值 |
| 必要時 | 留有例外的模糊條件 | 必要時才執行驗證 | 當 `validateInput=true` flag 設定時，執行驗證 | 狀態 |
| 建議（作為 AC 主語） | 建議類 AC 無法驗證 | 建議開發者使用快取機制 | 所有讀取操作必須先檢查本地快取（`cache.get(key)`） | 路徑 |

### 改寫規則

檢測到禁用字樣時，改寫為以下三類具體條件之一：

1. **數值條件**：精確的量化指標
   - 格式：`>= N`、`< N`、`== N`、`N±M%`、`between N and M`
   - 範例：「回應時間 < 100ms」、「失敗重試次數 <= 3」

2. **狀態條件**：系統/業務狀態的枚舉或布爾值
   - 格式：`status=<value>`、`flag=<boolean>`、`exists=<boolean>`
   - 範例：「當 `status=completed` 時」、「若 `isAdmin=true`」

3. **路徑條件**：檔案/指令執行的具體事件或結果
   - 格式：「檔案 X 存在」、「指令 Y 執行成功」、「API endpoint /path 返回 HTTP 2xx」
   - 範例：「檢查 `~/.ssh/id_rsa` 是否存在」、「執行 `git status` 成功（exit code 0）」

### PO Round 1 自檢步驟

PO 在輸出 AC 表格前，執行以下自檢流程：

1. 提取 AC 欄位內容（所有候選 Story 的 AC 文本）
2. 執行 grep 自檢指令：
   ```bash
   # 檢測禁用字樣（任何匹配即為需改寫）
   grep -E '考慮|明確|適當|合理|盡量|儘可能|必要時|建議' <<< "$ac_text"
   ```
3. 若有匹配，逐一列出含禁用字樣的 AC，並在 Round 1 回傳表格中標記「**軟性字樣待改寫**」
4. 若無匹配，AC 通過自檢，可輸出表格

> **目的**：在 PO Round 1 階段提前攔截軟性字樣，避免進入 QA Round 3 後的返工。
