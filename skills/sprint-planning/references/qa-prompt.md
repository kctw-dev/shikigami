...
<!-- Sprint 182 retro action A2：將 PO 防護機制延伸至 QA 角色 -->

QA 在輸出 **AC 補捉表、隱性需求說明、驗收記錄、Challenge 表述** 等任何結構化文件前，**必須**執行自檢步驟確保內容不含軟性字樣。模糊的 AC 是測試無法執行的根因；模糊的 Challenge 表述會被 Architect/PO 駁回。

> **單一來源**：禁用字樣清單與改寫規則的權威定義在 [`po-prompt.md` §禁用軟性字樣清單（#994）](./po-prompt.md)。下方為 QA 視角的對照與自檢流程。

### 禁用字樣（與 PO 同份 8 個詞）

| 禁用字樣 | QA 常見誤用情境 | 改寫示例 |
|---------|--------------|---------|
| 考慮 | 「測試應考慮邊界情境」 | 「測試覆蓋：最小值 = 0、最大值 = INT_MAX、空字串、null」 |
| 明確 | 「錯誤訊息應明確」 | 「錯誤訊息含 errorCode（4 位數）+ 中文說明 + correlation_id」 |
| 適當 | 「適當的測試策略」 | 「單元測試覆蓋 ≥ 80%，整合測試覆蓋主路徑 + 3 個錯誤路徑」 |
| 合理 | 「合理的回應時間」 | 「P95 < 200ms，P99 < 500ms（在 100 RPS 負載下）」 |
| 盡量 | 「盡量補捉隱性需求」 | 「對每個 Story 至少追問 3 個隱性 AC：效能、錯誤處理、資料完整性」 |
| 儘可能 | 「儘可能完整測試」 | 「測試矩陣：browser × OS × locale = 必須覆蓋的 9 組合（詳列）」 |
| 必要時 | 「必要時執行 challenge」 | 「Architect 估 = S 但實作含 ≥ 2 個檔案 → 觸發 Challenge Round 1」 |
| 建議 | 「建議補上負面測試」 | 「**必須**補上負面測試（無 → DoD NOT_READY）」 |

### QA Round 3 自檢步驟

每次輸出 AC 補捉表 / 驗收記錄 / Challenge 訊息前：

1. 將候選文字組合為 `$qa_text`
2. 執行 grep 自檢：
   ```bash
   # 檢測禁用字樣（任一匹配即為需改寫）
   echo "$qa_text" | grep -E '考慮|明確|適當|合理|盡量|儘可能|必要時|建議'
   ```
3. 若有匹配 → 在輸出末尾標記 `[NEEDS_REVISION] 含禁用軟性字樣：<逐項列出>`，並回退至補捉/挑戰前的步驟重寫
4. 若無匹配 → 輸出通過自檢，可進入下一步

### Challenge 表述特別規範

QA 對 Architect 的挑戰若使用「考慮」「建議」「適當」等軟性字樣，會被 Architect 視為**非具體挑戰**而合理駁回，導致 Challenge 失效。

正確 Challenge 應為：

```
[CHALLENGE] Story #N 估點 = S
反證：受影響檔案數 = 4（檢視 AC1 / AC3 / NFR2），歷史 4 檔變更平均 = M（見 docs/sprints/sprint_165.md §retro Story B）
請求 Architect 重估或補充估點理由。
```

而非：

```
[CHALLENGE] Story #N 估點建議再考慮一下，盡量更合理一些  ← 將被駁回
```
