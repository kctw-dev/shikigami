...
if grep -q "關聯 PR comments 掃描步驟" "$SKILL_FILE"; then
  ok "AC1-1：cruise SKILL.md 包含關聯 PR comments 掃描步驟段落"
else
  fail "AC1-1：cruise SKILL.md 缺少關聯 PR comments 掃描步驟段落"
fi

# AC1-2：掃描步驟應位於「留言掃描步驟」之後
...
if grep -q "gh pr view" "$SKILL_FILE"; then
  ok "AC2-1：cruise SKILL.md 包含 gh pr view 指令"
else
  fail "AC2-1：cruise SKILL.md 缺少 gh pr view 指令"
fi

# AC2-2：應包含 comments,reviews 欄位擷取
if grep -q "comments,reviews" "$SKILL_FILE"; then
...
if grep -q "LAST_PATROL_TIME" "$SKILL_FILE"; then
...
if grep -q "timelineItems" "$SKILL_FILE"; then
...
if grep -q "PR comments 與 Issue comments 衝突時的優先級規則\|PR comments 與 Issue comments.*衝突" "$SKILL_FILE"; then
...
if grep -q "以 PR comments 為準" "$SKILL_FILE"; then
...
if grep -q "Step 0.5" "$SKILL_FILE"; then
...
if grep -q "pr-instruction-overrides-issue" "$SKILL_FILE"; then
...
# AC4-1：文件應有回歸測試場景描述
if grep -q "回歸測試場景" "$SKILL_FILE"; then
...
if grep -q "LinGeorge2/AIO-System" "$SKILL_FILE" && grep -q "PR #29" "$SKILL_FILE"; then
...
if grep -q "拆分" "$SKILL_FILE" && grep -q "催促合併\|不催促合併" "$SKILL_FILE"; then
