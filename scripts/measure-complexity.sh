...
if [[ "$MODE" == "baseline" ]]; then
  ...
  echo ""
  printf "%-20s %6s  （門檻 %s）\n" "SKILL_COUNT" "$SKILL_COUNT" "$COMPLEXITY_SKILL_BUDGET"
  printf "%-20s %6s  （門檻 %s）\n" "AGENT_COUNT" "$AGENT_COUNT" "$COMPLEXITY_AGENT_BUDGET"
  printf "%-20s %6s  （門檻 %s）\n" "HOOK_COUNT"  "$HOOK_COUNT"  "$COMPLEXITY_HOOK_BUDGET"
  printf "%-20s %6s  （門檻 %s）\n" "TOTAL_LINES" "$TOTAL_LINES" "$COMPLEXITY_LINES_BUDGET"
  echo ""
