...
本節記錄 US-116（UIUX Agent 模型分層策略調查）的調查結論，並作為 US-136（模型分層策略實作規劃）的依據。

### 任務複雜度分析（AC1）

三層管線各層的任務特性與複雜度如下：

| 管線層 | 任務類型 | 推理需求 | 創意需求 | 驗證需求 | 綜合複雜度 |
|--------|---------|---------|---------|---------|-----------|
| **UX Agent** | 語意化資訊架構萃取 | **高**（從非結構化自然語言推導頁面架構、元件層級、互動行為） | **中**（需設計判斷力選取 semanticRole 與 layoutHint） | 低（Schema 驗證由後續步驟處理） | **高** |
| **UI Agent** | 前端代碼生成 | **低**（輸入已高度結構化，SSD JSON 消除語意歧義） | **低**（嚴格的元件庫白名單限制創意空間） | **中**（規則遵循：Design Token 注入、白名單約束） | **中** |
| **Vision Critic** | 多模態視覺審查 | **中**（對照 SSD 規格識別截圖中的偏差，需跨模態理解） | 低（審查任務，非生成任務） | **高**（WCAG 對比度計算、元件位置偏移量化、間距合規性判定） | **中高** |

**關鍵洞察**：

1. **UX Agent 是推理密集層**：從「As a user, I want...」的模糊自然語言萃取出具體的版面規格（sections、component hierarchy、interactions），需要深層語意理解，是三層中推理要求最高的任務。

2. **UI Agent 是規則遵循層**：輸入（SSD JSON）已是機器可讀的結構化格式，核心挑戰在「嚴格遵循規則」而非「深度推理」。高結構化輸入降低了對高階模型的需求。

3. **Vision Critic 是多模態驗證層**：輸入包含截圖（圖片）和骨架文件（JSON），需多模態能力，但主要任務是量化評分而非創意推理，對比 UX Agent 的模型需求略低，但有「vision 能力必要約束」。
...
| 優先級 | 模型 | 適用場景 | 成本/品質權衡 |
|--------|------|---------|-------------|
| 1（首選高品質） | `claude-opus-4` | 複雜 User Story（多頁面、複雜互動、Edge Case 豐富）；生產環境；設計評審關鍵節點 | 高成本（約 Sonnet 15 倍），最高語意理解品質。高推理任務值得投資。 |
| 2（標準） | `claude-sonnet-4-6` | 單頁面標準 Story；CI 自動化；原型快速迭代 | 中成本，高品質。日常開發的最佳性價比選擇。 |

**成本/品質權衡說明**：UX Agent 是管線的「品質乘數」—— UX Agent 輸出品質低劣時，下游所有層的成本（包括 UI Agent 重工和 Vision Critic 多輪退件）都會放大，因此高複雜度場景選用 Opus 的長期成本效益更高。

#### UI Agent 推薦模型

| 優先級 | 模型 | 適用場景 | 成本/品質權衡 |
|--------|------|---------|-------------|
| 1（首選） | `claude-sonnet-4-6` | 標準前端代碼生成；CI 自動化管線；退件重試（最多 3 次） | 中成本，高品質。規則遵循任務 Sonnet 已充分勝任，無需 Opus 的高推理能力。 |
| 2（備選） | `claude-opus-4` | SSD 元件 ≥ 20 個；複雜互動狀態機；同一輸入第 2 次退件後升級 | 高成本，適合系統性退件問題的突破嘗試。 |

**成本/品質權衡說明**：UI Agent 輸入已由 UX Agent 完成語意轉換，代碼生成任務的主要挑戰是「映射準確性」而非「推理深度」。Sonnet 4.6 在結構化代碼生成場景具高性價比，是 CI 預設選擇。

#### Vision Critic Agent 推薦模型

| 優先級 | 模型 | 適用場景 | 成本/品質權衡 |
|--------|------|---------|-------------|
| 1（首選） | `claude-sonnet-4-6` | CI 自動化視覺驗證；標準頁面截圖；退件重試迴圈 | 中成本，高品質。已驗證多模態 vision 能力（OQ-1 PoC）。標準選擇。 |
| 2（高精度） | `claude-opus-4` | 複雜 UI 截圖（元件 ≥ 25 個）；WCAG 對比度邊界案例；法規合規場景；同一 Story Hard Gate 退件 ≥ 2 次 | 高成本，最高視覺解析精度。適合細節密集的截圖審查場景。 |

**Vision 能力必要約束（不可妥協）**：Vision Critic 的任何推薦或替代模型，必須支援 `image` 類型多模態輸入（Base64 PNG 傳遞）。不支援 vision 的模型絕對不能用於 Vision Critic Agent，否則截圖輸入無法處理，管線失敗。

**成本/品質權衡說明**：Vision Critic 的截圖本身佔用大量 token（3,000–8,000 tokens），使用 Opus 的成本會進一步放大。Sonnet 4.6 已驗證可勝任標準視覺審查，建議以 Sonnet 為 CI 預設，在精確性需求高時才切換 Opus。

### 三層管線模型選擇矩陣

| 判斷維度 | 全線使用 Opus | 全線使用 Sonnet | 混合策略（建議） |
|---------|------------|--------------|----------------|
| 成本 | 最高（Opus 約 15x Sonnet） | 最低 | 中等（視複雜度動態切換） |
| 品質 | 最高 | 高（規則遵循類任務差距不大） | 複雜推理任務高品質 + 代碼生成任務節省成本 |
| CI 適用性 | 低（成本過高，不適合每次 PR 觸發） | 高 | 高（CI 預設 Sonnet，手動觸發可選 Opus） |
| YAGNI 符合性 | 不符合（過度投資） | 符合 | 符合（按需升級） |

**建議策略（混合策略）**：

- **CI 自動化管線（每次 PR 觸發）**：三層全部使用 `claude-sonnet-4-6`
- **手動開發模式（設計評審、主要 UX 頁面）**：UX Agent 使用 `claude-opus-4`，其餘維持 Sonnet
- **退件降級升級規則**：UI Agent 或 Vision Critic 在第 2 次退件後，同層切換至 Opus 嘗試突破

### Figma 架構潛在影響評估

若未來採用 Figma 直接操作架構（ADR-015 Proposed），模型分層策略的調整方向：

| 架構層 | 現行 SSD JSON 管線需求 | Figma API 操作需求 | 模型需求差異 |
|--------|---------------------|-----------------|------------|
| UX 規格層 | 高推理（語意萃取 → SSD JSON） | **更高推理**（自然語言 → Figma 元件 API 呼叫序列，需理解 Figma 資料模型） | Figma 架構對 UX 層推理需求更高，更需要 Opus |
| UI 實作層 | 中等（規則映射 → 代碼） | **中等**（生成 Figma 元件屬性設定，與代碼生成複雜度相近） | 需求相近，Sonnet 仍適用 |
| 視覺驗證層 | 多模態截圖審查 | **可能簡化**（Figma 可直接提供元件規格，無需截圖比對） | Figma 架構下 Vision Critic 可能轉型為「規格符合性驗證」而非「視覺截圖審查」，降低多模態需求 |

**結論**：不論最終採用哪種架構，本節的模型分層策略框架（高推理任務使用高階模型、代碼生成任務使用成本優化模型）保持有效。Figma 架構的具體影響待 ADR-015 決策後補充。
