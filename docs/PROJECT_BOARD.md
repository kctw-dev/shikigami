...
| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| feat: 解決 LLM 規則衰減 — 外部狀態機 + 短命 Agent 架構 | #962 | L | 3 | DONE (PR#964) |
| retro: sprint-candidate label 應於 Sprint Planning 選入時自動移除 | #954 | S | 1 | DONE (PR#963) |
| retro: 評估 Discovery/RESEARCH Story 序列依賴優化機會 | #935 | M | 2 | DONE (PR#966) |
...
| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| feat: Hook 執行超時與隔離機制 | #923 | M | 2 | DONE (#936, b895904) |
| chore: 建立 Hook 開發標準規範 | #924 | M | 2 | DONE (#937, b264772) |
| test: Hook 整合測試補齊 | #925 | M | 2 | DONE (#938, 6b8d27f) |
| retro: sprint-candidate 水位持續監控與補充機制 | #934 | S | 1 | DONE (#943, 9dc2994) |
...
> **結果**：Goal 達成（4/4 Stories DONE）。Velocity 5 pts，完成率 100%。

| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| retro: Backlog Discovery 補充 -- sprint-candidate 水位低於閾值 | #919 | S | 1 | DONE |
| retro: complexity-trend.sh 定期 CI 觸發機制 | #922 | S | 1 | DONE (#931) |
| retro: Backlog Discovery 流程最佳化 | #887 | S | 1 | DONE (#932) |
| feat: Retro Action Items 歷史分析工具 | #872 | M | 2 | DONE (#933) |
...
| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| feat: Backlog 健康度儀表板 — sprint-candidate 數量/年齡/MoSCoW 分布一覽 | #824 | S | 1 | DONE (#832) |
| feat: Sprint Velocity Trend 自動報告 — 容量估算資料化（最近 5 Sprint 滾動平均） | #825 | S | 1 | DONE (#833) |
| feat: RICE Score 缺漏掃描 — sprint-candidate 補分提醒腳本 | #826 | S | 1 | DONE (#834) |
| feat: Sprint Retrospective 自動模板生成 — 預填 Sprint 指標減少手工填寫 | #827 | S | 1 | DONE (#835) |
...
> **結果**：Goal 達成（5/5 Stories DONE）。Velocity 7 pts，完成率 100%。
> **容量**：7 pts
...
| feat: Context Engineering JIT Skill 載入策略（減少 context 壓力） | #793 | S | 1 | DONE (#805) |
| feat: Schema-First 強制工具 — API Contract 前置驗證腳本（ADR-036 落地） | #802 | S | 1 | DONE (#806) |
| feat: Quick Ship Pipeline — 統一 lint/test/bump/commit/PR 單一指令交付 | #798 | S | 1 | DONE (#807) |
| feat: Quality Observer MCP Phase 2 — metrics 端點強化（coverage/debt/health） | #794 | S | 1 | DONE (#808) |
| feat: Session Watchdog — 存活監控心跳機制（無聲 hang 偵測） | #772 | S | 1 | DONE (#809) |
...
> **結果**：Goal 達成（6/6 Stories DONE）。Velocity 6 pts，完成率 100%。
> **容量**：6 pts

| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| feat: Kill Switch — 優雅緊急停止機制（Story 邊界安全中止自動執行） | #783 | S | 1 | DONE (#784) |
| feat: DM-4 Write Gateway 系統化 — 平行 subagent 共用文件寫入保護 | #775 | S | 1 | DONE (#786) |
| feat: quality-gate 決策記錄機制 — CRITICAL 發現處置台帳 + 防濫用稽核腳本 | #779 | S | 1 | DONE (#788) |
| spike: Playwright MCP subagent 穩定性驗證（Browser Automation 前置 Spike） | #774 | S | 1 | DONE (#785) |
| feat: ADR 目錄索引自動維護 v2 — docs/adr/README.md 可搜尋索引 | #778 | S | 1 | DONE (#787) |
| feat: quality-gate CRITICAL 互動決策點 — 結構化風險接受 / 拒絕 / 延後機制 | #773 | S | 1 | DONE (#789) |
...
| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| feat: Sprint Review 邊界案例測試自動化（test-sprint-review-boundary.sh） | #732 | S | 1 | DONE(#761) |
| feat: ADR 目錄索引自動維護（docs/adr/README.md 自動更新） | #742 | S | 1 | DONE(#762) |
| feat: Sprint Planning 會議紀錄模板標準化（templates/sprint-planning-meeting.md） | #744 | S | 1 | DONE(#763) |
| feat: Sprint Review 指標收集平行化（Analytics + SPACE + Quality Observer 同步執行） | #709 | M | 3 | DONE(#764) |
| feat: ADR-039 Model Routing 準確率追蹤 Dashboard | #711 | S | 1 | DONE(#765) |
...
| retro: Sprint 153 Backlog 補充頻率調整 — 建立提前預警機制 | #721 | S | 1 | DONE(#728) |
...
| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| retro: 修復 #704 Issue body 截斷問題（AC/NFR 內容遺失） | #707 | S | 1 | DONE(closed) |
| retro: Sprint 151 PO Backlog Discovery — 補充 sprint-candidate 至 >= 8 | #706 | M | 3 | DONE(closed) |
| retro: /shikigami:doctor cruise 定期觸發整合（AC6 補完） | #704 | S | 2 | DONE(#714) |
...
> **結果**：Goal 達成（3/3 Stories DONE）。Velocity 4 pts，完成率 100%。連續第 24 Sprint 100%（127-150）。
> **容量**：4 pts

| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| fix: cruise 啟動時自動清理 stale worktree（prunable） | #697 | S | 1 | DONE(#700) |
| feat: /shikigami:doctor — 消費端專案健康診斷（檢查→問診→處置→追蹤） | #693 | M | 2 | DONE(#701) |
| retro: bug 類 Issue Template 補充 NFR 欄位 — 與 enhancement 模板對齊 | #699 | S | 1 | DONE(#702) |
...
> **結果**：Goal 達成（3/3 Stories DONE）。Velocity 4 pts，完成率 100%。連續第 23 Sprint 100%（127-149）。
> **容量**：4 pts

| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| fix: onboarding 流程缺少 claim/release hooks 安裝步驟 | #692 | S | 1 | DONE(#695) |
| retro: Sprint Review §1.5 加入 ROADMAP.md 版號一致性硬性檢查 | #690 | S | 1 | DONE(#694) |
| retro: Backlog Bridge 流程更新 — 引導新 Issue 使用模板 | #691 | M | 2 | DONE(#696) |
...
| refactor: cruise/SKILL.md 行數超限重構 — 段落移至 references/ 控制在 250 行以內 | #684 | M | 2 | DONE(#689) |
...
> **結果**：Goal 達成（3/3 Stories DONE）。Velocity 5 pts，完成率 100%。連續第 21 Sprint 100%（127-147）。
> **容量**：5 pts

| Story | Issue | Size | Points | 狀態 |
|-------|-------|------|--------|------|
| retro: Backlog 補充 — Sprint 146 後 Backlog 清空，建立新 sprint-candidate | #679 | M | 2 | DONE |
| feat: doc_only Story 內容品質審查 — Reviewer + Challenger 制衡 | #680 | M | 2 | DONE(#686) |
| feat: SRE 巡檢讀取 runner_min_count 設定，避免 MIG 正常縮減誤報 | #681 | S | 1 | DONE(#685) |
