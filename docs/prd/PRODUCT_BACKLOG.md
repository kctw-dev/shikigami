...
| 排序 | Story | RICE | MoSCoW | Size | 來源 | 狀態 |
|------|-------|------|--------|------|------|------|
| 1 | Retro #47：短衝模式設計與實作 | 待定 | Should | 待定 | Sprint 17 Retro / Issue #47 | [OVERDUE-SYNC] Issue #47 已關閉（CLOSED，2026-03-02），對應 US-31（Sprint 20 Done），Backlog 狀態未同步 — 應移除或標記 Done |
...
### Sprint 19 Retro — 新增 Action Items
...
### Sprint 33 — 已完成 Stories

| 排序 | Story | RICE | MoSCoW | Size | 來源 | 狀態 |
|------|-------|------|--------|------|------|------|
| 1 | US-63：Issue #46 子 Story #4 — 需求入庫自動化（PO Backlog Intake cron + shikigami:backlog-management Skill） | 待定 | Should | M | GitHub Issue #46 子 Story #4 / Sprint 33 Planning | Done（Sprint 33） |
| 2 | US-64：M5 條件 (a) 主動觸及強化 — 外部社群推廣文案製作（GitHub README badges + 技術文章草稿 + 主動 outreach 指引） | 待定 | Should | S | M5 條件 (a) 外部使用者觸及 / Sprint 33 Planning | Done（Sprint 33） |
...
| 排序 | Story | RICE | MoSCoW | Size | 來源 | 狀態 |
|------|-------|------|--------|------|------|------|
| 1 | US-60：Issue #46 子 Story #3 — 排程衝刺程式碼入庫 QA 自動化（schedule SKILL.md + scrum-master/sprint-review SKILL.md） | 待定 | Should | M | GitHub Issue #46 子 Story #3 / Sprint 32 Planning | Done（Sprint 32） |
| 2 | US-61：M5 條件 (a) 外部使用者觸及強化 — Onboarding 低摩擦路徑最佳化（README + tutorial + M5 追蹤更新） | 待定 | Must | S | M5 條件 (a) 外部使用者觸及 / Architect 建議（Sprint 32 Planning） | Done（Sprint 32） |
| 3 | US-62：Issue #35 — Token 追蹤 Baseline Snapshot 機制（Metrics_Log.md + sprint-planning/execution SKILL.md） | 待定 | Should | S | GitHub Issue #35 / Sprint 32 Planning | Done（Sprint 32） |
...
| 排序 | Story | RICE | MoSCoW | Size | 來源 | 狀態 |
|------|-------|------|--------|------|------|------|
| 1 | US-54：互動 Session 自動偵測待審排程 PR + Scrum Master 提醒機制（Issue #46 子 Story #1） | 待定 | Should | S | GitHub Issue #46 / Sprint 30 Planning | Done（Sprint 30） |
| 2 | US-55：README 準確性修正 — 版本號、Skill 數量、版本歷史對齊 | 待定 | Must | S | Architect 建議（Sprint 30 Planning） | Done（Sprint 30） |
| 3 | US-56：Deployment Readiness 版本 Tag 決策規則強化（Issue #36） | 待定 | Must | M | GitHub Issue #36 / Architect 建議（Sprint 30 Planning） | Done（Sprint 30） |
...
| 排序 | Story | RICE | MoSCoW | Size | 來源 | 狀態 |
|------|-------|------|--------|------|------|------|
| 1 | US-47：ADR-008: OpenCode 平台整合策略架構決策 | 待定 | Must | S | US-46 Phase 1 建議 / OPENCODE_POC.md §8.4 | Done（Sprint 27） |
| 2 | US-48：OpenCode Phase 2 — Subagent 角色移植與派遣驗證 | 待定 | Must | L | OPENCODE_POC.md Phase 2 定義 / M5 外部使用者觸及 | Done（Sprint 27） |
...
| 排序 | Story | RICE | MoSCoW | Size | 來源 | 狀態 |
|------|-------|------|--------|------|------|------|
| 1 | US-43：M5 完成條件終審 + Issues #3/#4/#5 結論決策 | 待定 | Must | M | ROADMAP M5 / v1.0.0 前置條件 | Done（Sprint 25） |
| 2 | US-44：Tech Debt Grooming Sprint 25 + TD-001 降級決策 | 待定 | Should | S | Tech_Debt_Registry.md / Sprint 23 | Done（Sprint 25） |
| 3 | US-45：OpenCode POC 可行性調查 | 待定 | Could | S | Retro #42 / Issue #3 / US-17 後續行動 | Done（Sprint 25） |
...
| 排序 | Story | RICE | MoSCoW | Size | 來源 | 狀態 |
|------|-------|------|--------|------|------|------|
| 1 | OpenCode POC 可行性調查 | 待定 | Could | S | Retro #42 / Issue #3 | Done（US-45，Sprint 25） |
...
**MoSCoW**：Should
**Size**：L / **Points**：3（Architect 調降 AC3/AC4 子集為 S/1pt，排入 Sprint 12）
**備注**：AC2 的模型指定可能需要 ADR（是否屬於技術選型），Sprint Planning 時由 Architect 判斷

**Sprint 12 排程說明**：
- **AC3/AC4（量測）**：排入 Sprint 12，Size S / 1pt。Architect 確認為純量測，無 ADR 需求，Hard Gate PASS。
- **AC1（檔案傳遞模式）**：Sprint 11 US-25 已實現等效架構，暫緩獨立執行，後續視量測結果決定是否補實作。
- **AC2（輕量模型指定）**：暫緩，待 AC3/AC4 量測結果確認優化空間後再排期。
- **基準值**：Sprint 10 Planning token = 87M（Metrics_Log.md 分環節記錄）；Sprint 12 Planning 總 token 需 < 43.5M（降幅 >= 50%）。
